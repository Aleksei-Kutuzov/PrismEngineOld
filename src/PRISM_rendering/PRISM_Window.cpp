...
PRISM_Window::PRISM_Window(int width, int height, const char* title, const char* vertexShaderSource, const char* fragmentShaderSource)
    : window(nullptr), glContext(nullptr), running(true)
{
    this->w = width;
    this->h = height;
    ...
    shaderManager.initShaders(vertexShaderSource, fragmentShaderSource);
}
...
void PRISM_Window::handleEvents(void(*op)(SDL_Event)) {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        op(e);
        if (e.type == SDL_QUIT) {
