...
void test_Zbuff() {
	...
	std::string savePath = getInternalStoragePath() + "/texture.png";
	saveTexture(renderer, texture, savePath);
    while (!done) {
...
void main_(){
    ...
    AbstractCamera3D MainCamera(0.1, 1000.0f, 90.0f,
                                mode.w, mode.h);
	MainCamera.SetPosition({0, 8, 1}, {0, 0, 0}, {0, 0, 1});
    struct Vector3d vectcube = {0, 0, 8};
...
//    Obj1.SetMesh(meshObj1);
    Obj1.DownloadFromOBJ("hause.obj");
...
//        ftimeline ++;
        float jx, jy;
        GetJoystickPosition(joysticks[0], jx, jy);
		float jx2, jy2;
        GetJoystickPosition(joysticks[1], jx2, jy2);
        struct Vector3d vectrotcube = {0.0f, jx * 180 * 0.005f, jy * 180 * 0.005f};
		struct Vector3d vectposcube = {jx2 * 0.5f, jy2 * 0.5f, 5.0f};
        SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255);
        SDL_RenderClear(renderer);
//		Obj1.Rotation.y += {0, 1, 0};
//		Obj1.Rotation.z += {0, 0, 0};
//        Obj1.SetRotateXYZ(Obj1.Rotation);
		Obj1.Coordinate.x += vectposcube.x;
		Obj1.Coordinate.y += vectposcube.y;
		Obj1.SetMoveXYZ(Obj1.Coordinate);
...
//        Obj2.SetRotateXYZ(Obj2.Rotation);
		Obj2.Coordinate.x -= vectposcube.x;
		Obj2.Coordinate.y -= vectposcube.y;
		Obj2.SetMoveXYZ(Obj2.Coordinate);
...
//
		PrismRender.ReturnFrameTexture(mode.w, mode.h, {DisplayDimming1.active, DisplayDimming2.active, DisplayTriangleContours.active, Zbuffer.active, false, true, 5});
...
void renderTriangle() {
    glBegin(GL_TRIANGLES);
        ...
        glVertex2f(0.0f, 0.5f); // Верхний угол
    glEnd();
}
...
int GL_test() {
    ...
    // Создание окна
    SDL_Window* window = SDL_CreateWindow("Отрисовка треугольника с помощью SDL и OpenGL",
