...
private:
    ...
    /// </summary>
    PRISM_Vector3d upDir;

    /// <summary>
    /// Обновляет направление взгляда камеры на основе текущих углов вращения.
    /// </summary>
...
public:
    ...
    AbstractCamera3D(float Near = 0.1f, float Far = 1000.0f, float Fov = 90.0f, int displW = 800, int displH = 480)
        ...
        FFovRad(1.0f / tanf(Fov * 0.5f / 180.0f * 3.14159f)),
        ScreenW(displW), ScreenH(displH),
        z_buffer(ScreenW, ScreenH) {
        // Инициализация векторов
        Translate = { 0, 0, 0 };
        Rotation = { 0, 0, 0 };
        viewDir = { 0, 0, 1 }; // Камера смотрит вдоль оси Z
        upDir = { 0, 1, 0 };   // Вектор "вверх" направлен вдоль оси Y
        UpdateViewDirection(); // Инициализация направления взгляда
    }
